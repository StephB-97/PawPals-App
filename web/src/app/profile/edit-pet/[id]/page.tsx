    ...
    <main className="min-h-screen bg-[#FDF6EE] md:px-6 md:py-10">
      <div className="mx-auto w-full max-w-md bg-white min-h-screen md:min-h-0 md:max-w-4xl md:rounded-2xl md:shadow-sm">
        <div className="flex items-center border-b border-[#E8DDD0] gap-[0.9rem] pt-12 px-5 pb-3 md:px-6">
          <span
            className="text-[1rem] text-[#1A1A2E] cursor-pointer lg:hidden"
            onClick={() => router.push("/profile")}
          >
            &larr;
          </span>
          <span className="text-[1rem] text-[#1A1A2E] md:text-[1.1rem] font-bold">Edit Pet</span>
        </div>
        <form onSubmit={handleSubmit} className="p-5 md:p-8">
          <div className="flex flex-col gap-6 md:grid md:grid-cols-[260px_1fr] md:gap-8">
            <ImageUpload
              onUploaded={(url) => setFormData((prev) => ({ ...prev, photoUrl: url }))}
            />
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex gap-3">
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 py-3 px-4 border border-[#E8734A] bg-[linear-gradient(135deg,_#FF6B6B,_#FF8C42)] text-white rounded-lg cursor-pointer font-semibold disabled:opacity-50"
                >
